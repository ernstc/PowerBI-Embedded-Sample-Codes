...
namespace EmbedAPISample
{
    class Program
    {
        ...
        private static Guid groupId = Guid.Parse("<WORKSPACE ID>");
        
        private static Guid reportId = Guid.Parse("<REPORT ID>");
        private static Guid datasetId = Guid.Parse("<DATASET ID>");
            ...
            try
            {
                ...
                using (var client = new PowerBIClient(new Uri(apiUrl), tokenCredentials))
                {
                    ...
                    else if(useEmbedToken && useRLS)
                    {
                        ...
                        var rls = new EffectiveIdentity(username: "<USERNAME>", new List<string> { datasetId.ToString() });
