...
{
    public class PbiEmbedService(AadService aadService)
    {
        private readonly AadService aadService = aadService;
        ...
        public async Task<EmbedParams> GetEmbedParams(Guid workspaceId, Guid reportId, [Optional] Guid additionalDatasetId)
        {
            ...
            else
            {
                ...
                if (additionalDatasetId != Guid.Empty && !datasetIds.Contains(additionalDatasetId))
                {
        ...
        public async Task<EmbedParams> GetEmbedParams(Guid workspaceId, IList<Guid> reportIds, [Optional] IList<Guid> additionalDatasetIds)
        {
            ...
            {
                datasetIds.AddRange(additionalDatasetIds.Where(d => !datasetIds.Contains(d)));
            }
